...
## 1. 概要
工学部一号館の建築側（東側）を対象に、CO2・温度・湿度などの環境情報や、LiDARを用いた二次元点群データによる人流情報、開閉センサーを用いたトイレの空室状況の情報を取得し、それらを用いて、使用者および管理者視点からの空間価値評価を試みる。また、リアルタイムな情報を学内限定で可視化することで、使用者の行動変容を促す。その様子を人流計測およびアンケート調査などから把握することで、空間価値評価手法の確立と、空間価値向上に努める。
...
<img src='images/lidarUnit.png' width='50%'>
...
- 過去データのダウンロードには、東京大学のメールアドレスを使用した認証システムを用いることを検討している。